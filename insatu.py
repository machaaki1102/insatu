...
excel_path = st.text_input("Enter Excel file path:", "your_excel_file.xlsx")
pdf_path = st.text_input("Enter output PDF file path:", "output.pdf")

# 行と列の範囲を入力
row_start = st.number_input("Start Row", min_value=1, value=1)
row_end = st.number_input("End Row", min_value=1, value=100)
col_start = st.number_input("Start Column (A=1, B=2, ...)", min_value=1, value=1)
col_end = st.number_input("End Column (A=1, B=2, ...)", min_value=1, value=7)
